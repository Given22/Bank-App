...
const datePassed = (date1, date2)=>Math.abs(date2 - date1) / 86400000;
...
const displayMovements = function(movements, currency, movD1, sort = false) {
    containerMovements.innerHTML = "";
    const mov2 = movements.slice();
    movements = sort ? movements.slice().sort((a, b)=>a - b) : movements;
    movD1 = sort ? sortD(movD1) : movD1;
    function sortD(movD) {
        const sorted1 = [];
        movements.forEach((mov)=>sorted1.push(movD[mov2.indexOf(mov)]));
        return sorted1;
    }
    movements.forEach(function(mov, i) {
        const type = mov > 0 ? "deposit" : "withdrawal";
        let date = new Date(movD1[i]);
        const now = new Date(Date());
        const passed = Math.round(datePassed(Date.parse(date.toISOString()), Date.parse(now.toISOString())));
        const optionsDate = {
            day: "numeric",
            month: "numeric",
            year: "numeric"
...
const createUserNames = function(accs) {
    accs.forEach(function(account) {
        account.username = account.owner.toLowerCase().split(" ").map((t)=>t[0]).join("");
...
const calcDisplaySum = function(movements, currency, i) {
    ...
    const incomes = movements.filter((movement)=>movement > 0).reduce((acc, curr)=>acc + curr);
    const inc = numberFormatter(incomes, optionsNumber);
    labelSumIn.textContent = `${inc} `;
    const out = Math.abs(movements.filter((movement)=>movement < 0).reduce((acc, curr)=>acc + curr));
    const o = numberFormatter(out, optionsNumber);
    labelSumOut.textContent = `${o} `;
    const interest = movements.filter((movement)=>movement > 0).map((dep)=>dep * i / 100).filter((int)=>int >= 1).reduce((acc, int)=>acc + int, 0);
    const intRest = numberFormatter(interest.toFixed(2), optionsNumber);
    labelSumInterest.textContent = `${intRest}`;
};
//______Transfer cash to other account
const transfer = function(user1, username, amount) {
    ...
    if (recipient) {
        ...
        if (amount <= user1.balance) {
            console.log(amount);
            recipient.movements.push(Math.round(amount * transfers.get(`${user1.currency}${recipient.currency}`)));
            recipient.movementsDates.push(new Date().toISOString());
            user1.movements.push(amount * -1);
            user1.movementsDates.push(new Date().toISOString());
            update(user1.movements, user1.currency, user1.movementsDates);
        }
    }
};
//______Take a loan
const takeLoan = function(user2, amount) {
    user2.movements.push(amount);
    user2.movementsDates.push(new Date().toISOString());
    update(user2.movements, user2.currency, user2.movementsDates);
};
//______Log in
const login = function(user3) {
    login__accounts.style.display = "none";
    containerApp.style.display = "grid";
    setTimeout(()=>containerApp.style.opacity = 100, 30);
    const movements = user3.movements;
    const currency = user3.currency;
    const interestRate = user3.interestRate;
    sorted = true;
    if (timer) clearInterval(timer);
    timer = startLogOutTimer();
    update(movements, currency, user3.movementsDates);
...
btnLogin.addEventListener("click", function(e) {
    ...
    user = accounts.find((user4)=>user4.username === username);
...
btnLoan.addEventListener("click", function(e) {
    ...
    if (amount > 0 && user.movements.some((mov)=>mov >= amount * 0.1)) takeLoan(user, amount);
...
btnSort.addEventListener("click", function() {
    if (!sorted) displayMovements(user.movements, user.currency, user.movementsDates, sorted);
    else displayMovements(user.movements, user.currency, user.movementsDates, sorted);
    sorted = !sorted;
});
